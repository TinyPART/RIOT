...
/* retrieve the task callback which is the third element of the secure
 * context
 */
static task_callback_t get_thread_callback(void)
{
    uint32_t *pctx = (uint32_t *)get_secure_context();

    return (task_callback_t)pctx[2];
}
 ...
 *     - returns the same value as the task function
 */
static void *secure_wrapper(void *arg)
{
    ...
    *((uint32_t *)&task_func) = (uint32_t)arg;
    ...
    /* and next the task callback */
    task_callback_t callback = get_thread_callback();
    if (callback != NULL) {
 ...
 * @return kernel_pid_t
 */
kernel_pid_t secure_thread( void *caller_context,
                            void *callee_context,
                            ...
                            const char *name )
{
    ...
    *context = (uint32_t)callee_context;
    context++;
    *context = (uint32_t)task_callback;
